...
def parse_args():
    ...
    parser.add_argument('--in_xml_vi_dir', default='./xml', help='可见光 XML 文件地址')
    # parser.add_argument('--in_xml_ir_dir', default='./ir_xml', help='红外光 XML 文件地址')
    parser.add_argument('--out_vi_txt_dir', default='./labels', help='可见光 TXT 文件地址')
    # parser.add_argument('--out_ir_txt_dir', default='./ir_labels', help='红外光 TXT 文件地址')
    parser.add_argument('--in_vi_img_dir', default='./images', help='裁切后的 vi-img 地址')
    # parser.add_argument('--in_ir_img_dir', default='./images2', help='裁切后的 ir-img 地址')
    ...
    with open(txt_file, "w+", encoding='UTF-8') as out_file:
        for obj in root.findall('object'):
            ...
            if obj.find('polygon'):
                ...
                data = str(min(1, max(0, result[0]))) + " " + str(min(1, max(0, result[1]))) + " " + str(min(1, max(0, result[2]))) + " " + str(min(1, max(0, result[3]))) + '\n'
                data = str(result_id) + " " + data
                out_file.write(data)
